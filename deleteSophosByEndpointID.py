import os
import requests

def authenticate(client_id, client_secret):
    url = "https://id.sophos.com/api/v2/oauth2/token"
    headers = {
        'Content-Type': 'application/x-www-form-urlencoded'
    }
    data = {
        'grant_type': 'client_credentials',
        'client_id': client_id,
        'client_secret': client_secret,
        'scope': 'token'
    }
    response = requests.post(url, headers=headers, data=data)
    if response.status_code == 200:
        auth_response = response.json()
        jwt_token = auth_response['access_token']
        print("Authentication successful. JWT token acquired.")
        return jwt_token
    else:
        print("Authentication failed.")
        return None

def find_tenant_id(jwt_token):
    url = "https://api.central.sophos.com/whoami/v1"
    headers = {
        'Authorization': f'Bearer {jwt_token}'
    }
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
        tenant_id = response.json()['id']
        data_region = response.json()['apiHosts']['dataRegion']
        print("Tenant ID found:", tenant_id)
        print("Data region found:", data_region)
        return tenant_id, data_region
    else:
        print("Failed to find Tenant ID.")
        return None, None

def read_endpoint_ids(file_path):
    with open(file_path, 'r') as file:
        endpoint_ids = file.readlines()
    endpoint_ids = [endpoint_id.strip() for endpoint_id in endpoint_ids]
    return endpoint_ids

def delete_endpoint(endpoint_id, token, data_region, tenant_id):
    url = f"{data_region}/endpoint/v1/endpoints/{endpoint_id}"
    headers = {
        'Authorization': f'Bearer {token}',
        'X-Tenant-ID': tenant_id
    }
    response = requests.delete(url, headers=headers)
    if response.status_code == 200:
        response_data = response.json()
        if response_data.get('deleted', False):
            print(f"Endpoint {endpoint_id} deleted successfully.")
        else:
            print(f"Failed to delete endpoint {endpoint_id}.")
            print("Response:", response_data)
    else:
        print(f"Failed to delete endpoint {endpoint_id}. Status code: {response.status_code}")
        print("Response:", response.text)


client_id = "CLIENT ID HERE"
client_secret = input("Enter CLIENT_SECRET: ")  
file_path = "new_computers.txt"

# Authenticate and obtain JWT token
jwt_token = authenticate(client_id, client_secret)

if jwt_token:
    tenant_id, data_region = find_tenant_id(jwt_token)
    
    if tenant_id and data_region:
        endpoint_ids = read_endpoint_ids(file_path)

        for endpoint_id in endpoint_ids:
            delete_endpoint(endpoint_id, jwt_token, data_region, tenant_id)